...
const App = () => {
  ...
  const enterBoardWord = (word) =>{
    ...
    if (matchCount === 5) { // 단어 맞췄을때
      ...
      setBoardData(newBoardData);
      localStorage.setItem("board-data", JSON.stringify(newBoardData));
    } else if (rowIndex+1 === 6) { // 6번 기회 다 썼을때(단어 못맞춤)
  ...
  return (
    <div className='container'>
      <div className='top'>
        <div className='title'>WordFit</div>
        {/* <button className="reset-board" onClick={resetBoard}>{"\u27f3"}</button> */}
      </div>
      {
        message && <div className='message' style={ message === "Completed!" ? {background:'#3978e9'} : {background:'#8974d4'}}>
          {message}
        </div>
      }
      <div style={{
        marginBottom:'25px', marginTop:'10px', display:'flex', 
        flexDirection:'row', gap:'5px'}}
      >
        <div style={{background:'#e96f6f', width:'30px', height:'30px'}}></div>
        <div style={{background:'#f3bc06', width:'30px', height:'30px'}}></div>
        <div style={{background:'#3db90a', width:'30px', height:'30px'}}></div>
      </div>
      <div className='cube'>
      ...
      </div>
      {
        ...
        ? (
          ...
          <div style={{textAlign: 'center', position: 'relative', bottom: '20px'}}>
            ...
            <input type='button' className='nextWord' value='NEXT WORD' onClick={()=>{resetBoard();}} />
          </div>
        )
        : ( <div className='bottom'><Keyboard boardData={boardData} handleKeyPress = {handleKeyPress}/></div> )
      }
    </div>
  )
}
